...
RSpec.describe User, type: :model do
  ...
  describe '会員情報登録' do
    context '新規登録できる場合' do
      it 'nickname、email、passwordとpassword_confirmation、last_name、first_name、last_name_kana、first_name_kana、birthが存在すれば登録できる' do
    ...
    context '新規登録できない場合' do
      ...
      it 'passwordが半角英数字でなければ登録できない' do
        @user.password = '1２345a'
        @user.password_confirmation = '1２345a'
        @user.valid?
        expect(@user.errors.full_messages).to include('Password には半角文字で英字と数字の両方を含めて設定してください')
      end
